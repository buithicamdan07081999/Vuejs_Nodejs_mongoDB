    ...
    <div class="categories">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div v-for="category in categories" :key="category.id" class="category-card">
          ...
          <p class="text-center text-gray-700 font-medium">{{ category.category }}</p>
          <p class="text-red-500 font-bold">{{ category.price.toLocaleString() }} VNĐ</p>
        <router-link class="text-blue-500 font-bold" :to="'/products/' + category.id">Xem chi tiết</router-link>
        </div>
      </div>
    </div>
  </div>
